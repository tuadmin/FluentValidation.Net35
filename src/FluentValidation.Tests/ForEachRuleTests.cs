...
namespace FluentValidation.Tests {
	...
	public class ForEachRuleTests {
		...
		[Fact]
		public async System.Threading.Tasks.Task Executes_rule_for_each_item_in_collection_async() {
			var validator = new TestValidator {
		...
		[Fact]
		public async System.Threading.Tasks.Task Correctly_gets_collection_indices_async() {
			var validator = new TestValidator {
				v => v.RuleForEach(x => x.NickNames).SetValidator(new MyAsyncNotNullValidator())
			};
		...
		[Fact]
		public async System.Threading.Tasks.Task Should_not_scramble_property_name_when_using_collection_validators_several_levels_deep_with_ValidateAsync() {
			var v = new ApplicationViewModelValidator();
...
#pragma warning restore 618
		...
		private async System.Threading.Tasks.Task<bool> ExclusiveDelay(int milliseconds) {
			lock (_lock) {
...
#pragma warning restore 618
		...
		[Fact]
		public async System.Threading.Tasks.Task When_runs_outside_RuleForEach_loop_async() {
			// Shouldn't throw an exception if the condition is run outside the loop.
			var validator = new InlineValidator<Tuple<Person>>();
